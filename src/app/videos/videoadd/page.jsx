...
function Page() {
        ...
        if (imageSizeInBytes > MAX_ACCEPTABLE_SIZE) {
          ...
          try {
            const response = await axios.post(
              `${baseURL}videos/upload-base64-image`,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (fileSizeInMB > 50) {
        alert(
          '영상 파일이 초과되었습니다. 15초 이하(50MB)의 영상을 선택하세요.',
        )
        setLoading(false)
        return
      }
      ...
      formData.append(
        'videoItems',
        Array.isArray(videoProductIds) ? videoProductIds : [videoProductIds],
      )

      const response = await axios
        .post(`${baseURL}videos/upload/${userId}`, formData, {
          headers: {
          ...
          <div className="pl-5">
            ...
            <div className="pl-5">
              ...
              : <p className="mt-5 text-xs">
                  (동영상 파일은 .mp4, .mov, .avi로 가능하며 사이즈는 50mb
                  이하로 업로드 가능합니다.)
                </p>
              }
            </div>
