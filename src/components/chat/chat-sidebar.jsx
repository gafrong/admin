...
function ChatItem({ id, name, image, message, time, queryType }) {
  ...
  return (
    <Link
      ...
      className="flex items-center gap-3 rounded-md p-2 hover:bg-muted/50 w-full"
      prefetch={false}
    >
      <ProfileImageSideBar image={imgSrc} name={name} />
      <div className="flex-1 min-w-0 overflow-hidden">
        <div className="flex items-center justify-between">
          <span className="truncate font-medium mr-2 flex-1">{name}</span>
          <span className="text-xs text-muted-foreground whitespace-nowrap flex-shrink-0">{time}</span>
        </div>
